...
def preprocessing(stopwords,tf_idf_index):
    try:
        ...
        with open("data_file_nolemma.json", "r") as write_file:
        ...
        with open("data_file_nolemma.json", "w") as write_file:
...
def query_processing(query):
    ...
    stopwords_file.close()
    alpha=0.05
    with open('data_file_nolemma.json') as f:
        tf_idf_index=json.load(f)
    print(len(tf_idf_index))
    with open('doc_matrix_nolemma.json') as f:
...
def document_matrix_creation():
    ...
    for word in words:
        ...
        index=index+1
    with open("doc_matrix_nolemma.json", "w") as write_file:
